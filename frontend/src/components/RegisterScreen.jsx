...
/**
 * Ro'yxatdan o'tish ekrani — SMS tasdiqlashsiz, bir qadamda:
 * ism + telefon → "Kirish" → sessiya localStorage'ga yoziladi → asosiy sahifa.
...
export default function RegisterScreen({ onSuccess, survey }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const user = await registerUser(name.trim(), phoneE164, survey);
      onSuccess(user);
    } catch (err) {
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-slate-500 mt-3 text-center leading-relaxed">
            ...
            <br />
            Davom etish uchun ism va raqamingizni kiriting.
          </p>
        </div>
