...
export default function CategoryRail({ categories, activeCategory, onSelect }) {
  return (
    <div className="glass-header sticky top-0 z-20 border-b border-slate-200">
      <div className="flex gap-2.5 sm:gap-3 overflow-x-auto scrollbar-hide px-4 py-2.5">
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className="cat-item group flex flex-col items-center gap-1 shrink-0 w-14 focus:outline-none"
              aria-pressed={active}
            >
              <span
                className={`cat-circle w-12 h-12 rounded-full flex items-center justify-center border ${
                  active
